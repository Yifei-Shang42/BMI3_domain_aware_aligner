...
"""
GREEDY MSA DOMAIN AWARE ALIGNER (UNFINISHED!!!!!!!!)
"""
def domain_aware_greedy_MSA(all_domains, ids, seqs, sigma=11, epsilon=1):
    ...
    :return: list of str final MSA results
    """
    # generate domain info for all sequences
    domain_infos = {}
    for prot_id in ids:
        domain_infos[prot_id] = sequence_to_domain_structure(prot_id, all_domains)
    # domain-based pairwise global alignment
    def init_best_pair_alignment_domain_based(seqs, sigma, epsilon, all_domains):
        """
        :param all_domains: dict preconstructed dictionary from UniProt
        :param seqs: list collection of multiple input protein sequences
        :param sigma: int penalty for opening a gap
        ...
        for pos1 in range(len(seqs) - 1):
            for pos2 in range(pos1 + 1, len(seqs)):
                ...
                align1, align2, curr_score = affine_gap_penalties_pair_wise_alignment(seq1, seq2, sigma, epsilon,
                                                                                      mode='pairwise')
    ...
    def add_one_seq_to_alignment_domain_based(unaligned_seqs, alignment, sigma, epsilon):
        """
        :param unaligned_seqs: list of str unaligned sequences :param alignment: list of str previous alignment sequences
        :param sigma: int penalty for opening a gap
        :param epsilon: int penalty for extending a gap
        ...
        for seq in unaligned_seqs:
            # for seq2 required in affine_gap_penalties_pair_wise_alignment we use a dummy sequence
            align1, align2_dummy, curr_score = affine_gap_penalties_pair_wise_alignment(seq, ''.join(
                'X' for _ in range(len(alignment[0]))),
                                                                                        sigma, epsilon, mode='profile',
                                                                                        alignment=alignment)
            if curr_score > best_score:
                best_align1 = align1
                best_align2 = align2_dummy
    ...
    best_pair, curr_align = init_best_pair_alignment_domain_based(seqs, sigma, epsilon)
    # record aligned seqs
    unaligned_seqs = [seq_ for seq_ in seqs if seq_ not in best_pair]
    # while not all aligned
    while unaligned_seqs:
        unaligned_seqs, curr_align = add_one_seq_to_alignment_domain_based(unaligned_seqs, curr_align, sigma, epsilon)
    return curr_align
